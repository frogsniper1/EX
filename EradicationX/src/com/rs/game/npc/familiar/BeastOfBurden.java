...
public class BeastOfBurden implements Serializable {
	...
	public void dropBob() {
		...
		for (int i = 0; i < beastItems.getSize(); i++) {
			...
			if (item != null)
				World.addGroundItem(item, WorldTile, player, null, false, -1, false);
		}
		beastItems.reset();
	}
	...
	public void addItem(int slot, int amount) {
		...
		if (!ItemConstants.isTradeable(item)
				|| item.getId() == 4049 || (familiar.canStoreEssOnly() && item.getId() != 1436 && item .getId() != 7936) || item.getDefinitions().getValue() > 50000) {
