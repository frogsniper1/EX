...
public class MercenaryMageCombat extends CombatScript {
	...
	public static final String[] ATTACKS = new String[] {
			"I will make you cry!", "Death is your only option!",
			"Why fight back?", "It is the time for you!",
			"IS THIS ALL!?" };
	...
	public int attack(final NPC npc, Entity target) {
		...
		if (attackStyle == 0) {
			...
			npc.setNextForceTalk(new ForceTalk("How u feel now ?"));
			...
			npc.setNextForceTalk(new ForceTalk("Is it cold?"));
			WorldTasksManager.schedule(new WorldTask() {
				int count = 0;
