...
public class BlinkCombat extends CombatScript {
	...
	private void attackSounds(NPC npc) {
		...
		case 1:
			...
			break;
		case 2:
			npc.setNextForceTalk(new ForceTalk("Twerk now!"));
			...
			public void run() {
				...
				for (Entity t : npc.getPossibleTargets()) {
					if (!t.withinDistance(npc, 3))
						continue;
					delayHit(npc, 1, t, new Hit(npc, getRandomMaxHit(npc, 450,
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (!specialRapidMagic && Utils.random(15) == 2 && !sprayAttack
				|| !specialRapidMagic && npc.getHitpoints() <= 13045
				&& !sprayAttack) {
			npc.setNextForceTalk(new ForceTalk(
					(npc.getHitpoints() <= 13045 ? "Ah! Grrr... Can't.. Stop me!"
							: "Aha!")));
				...
				public void run() {
					...
					for (Entity t : npc.getPossibleTargets()) {
						if (!t.withinDistance(npc, 10))
							continue;
						delayHit(npc, 1, t, new Hit(npc, getRandomMaxHit(npc, 210,
								NPCCombatDefinitions.MAGE, t),
								HitLook.MAGIC_DAMAGE));
						t.setNextGraphics(new Graphics(2855, 0, 0));
					}
					if (count++ >= (npc.getHitpoints() <= 13045 ? 13045 : 4)
							|| npc.isDead()) {
						stop();
