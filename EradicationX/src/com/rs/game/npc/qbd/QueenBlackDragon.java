...
public final class QueenBlackDragon extends NPC {
	...
	public void setPhase(int phase) {
		...
		case 5:
            ...
            if (attacker.getBossTask() != null) {
                if (attacker.getBossTask().getNPCId() == id) {
                    attacker.getSkills().addXp(Skills.SLAYER, attacker.getBossTask().getXPAmount());
                    attacker.getBossTask().decreaseAmount();
                    if (attacker.getBossTask().getTaskAmount() < 1) {
                    		if (!attacker.getBossTask().isTripled()) {
                            attacker.setBossSlayerPoints(attacker.getBossSlayerPoints() + 20);
                            attacker.sm("You have finished your slayer task, talk to Grim Reaper for a new task.");
                            attacker.sm("Grim Reaper rewarded you 20 Boss Slayer Points. You now have "
                                    + attacker.getBossSlayerPoints()
                                    + " Boss Slayer Points.");
                    		} else {
                                attacker.setBossSlayerPoints(attacker.getBossSlayerPoints() + 60);
                                attacker.sm("You have finished your slayer task, talk to Grim Reaper for a new task.");
                                attacker.sm("Grim Reaper rewarded you 60 Boss Slayer Points. You now have "
                                        + attacker.getBossSlayerPoints()
                                        + " Boss Slayer Points.");                    			
                    		}
                    		attacker.setBossSlayerCount(attacker.getBossSlayerCount(false) + 1);
                    		attacker.sm("You now have "+ attacker.getBossSlayerCount(false) +" boss slayer tasks completed.");
                            attacker.setBossTask(null);
                    } else {
                    attacker.getBossTask().setAmountKilled(attacker.getBossTask().getAmountKilled() + 1);
                    attacker.sm("You need to defeat "+ attacker.getBossTask().getTaskAmount() + " more "
                            + attacker.getBossTask().getName().toLowerCase() + " to complete your task.");
                    }
                    }
            }				
			break;
