...
public class DuelArena extends Controler {
	...
	private final WorldTile[] LOBBY_TELEPORTS = { new WorldTile(3367, 3275, 0),
			new WorldTile(3367, 3275, 0), new WorldTile(3367, 3275, 0),
			new WorldTile(3367, 3275, 0), new WorldTile(3367, 3275, 0),
			new WorldTile(3367, 3275, 0), new WorldTile(3367, 3275, 0),
			new WorldTile(3367, 3275, 0), new WorldTile(3367, 3275, 0),
	...
	private void openDuelScreen(Player target, boolean ifFriendly) {
		...
		player.getPackets().sendIComponentText(1310, 6, target.getDisplayName());
		player.getPackets().sendIComponentText(1310, 7, "Combat Level: ");
		player.getPackets().sendIComponentText(1310, 8, "    "+target.getSkills().getCombatLevelWithSummoning()+"");
		player.getPackets().sendIComponentText(1310, 9, "Killcount: ");
		player.getPackets().sendIComponentText(1310, 10,""+ target.getKillCount());
...
//		}
	...
	private void startEndingTeleport(Player player) {
		...
		player.setNextWorldTile(player.getLastWorldTile());
	}
	...
	private void beginBattle(boolean started) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					player.setNextForceTalk(new ForceTalk("Gl"));
	...
	public void process() {
		if (target.getControlerManager().getControler() == null) {
			endDuel(player, target);
		} else if (!target.getControlerManager().getControler().toString().contains("DuelArena")) {
			endDuel(player, target);
		} else if (plane != target.getPlane()) {
			endDuel(player, target);
		} else if (plane != player.getPlane()) {
			endDuel(target, player);		
		} else if (!target.withinDistance(player, 20)) {
			endDuel(player, target);			
		} else if (World.isPvpArea(target)) {
			endDuel(player, target);					
		}
	}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (loop == 0) {
					...
					endDuel(target, player);
					player.setNextAnimation(new Animation(1332));
				} else if (loop == 3) {
					this.stop();
				}
				loop++;
	...
	private WorldTile[] getPossibleWorldTiles() {
		...
		int[] arenaBoundariesX = { 3367, 3367, 3367 };
		int[] arenaBoundariesY = { 3367, 3367, 3367 };
