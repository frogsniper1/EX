...
public class CreationSkillsAction extends Action {
	...
	public static enum CreationObjects {
		...
		private CreationObjects(int baseTime, int randomLife, int randomTime,
				int level) {
	...
	public CreationSkillsAction(WorldObject object, Animation animation,
			Item bestItem, int baseId, int objectIndex) {
	...
	public int getSkillTimer(Player player, int skillId) {
		...
		int modifier = itemUsed.getDefinitions().getWearingSkillRequiriments()
				.get(skillId);
