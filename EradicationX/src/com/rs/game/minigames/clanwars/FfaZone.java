...
public final class FfaZone extends Controler {
	...
	public void start() {
		if (player.getSkills().getCombatLevel() < 70) {
			player.sendMessage("You need a combat level of at least 70 to fight.");
			return;
		}
		if (player.getEquipment().wearingThreeItems() < 4) {
			player.sm("You need to be wearing more than three items to fight.");
			return;
		}
			...
			public void run() {
				if (loop == 0) {
					...
					if (risk) {
						...
						if (killer != null) {
							killer.setAttackedByDelay(Utils.currentTimeMillis() + 12000);
	...
	public static boolean isOverloadChanged(Player player) {
		...
		return player.isCanPvp()
				&& ((FfaZone) player.getControlerManager().getControler()).risk;
	}
}