...
public class Bank implements Serializable {
	...
	private static final long MAX_BANK_SIZE = 468;
		...
		if (bankTabs != null) {
			for (int i = 0; i < bankTabs.length; i++) {
				for (int i2 = 0; i2 < bankTabs[i].length; i2++) {
					if (bankTabs[i][i2].getDefinitions().isNoted()) {
						bankTabs[i][i2].setId(bankTabs[i][i2].getDefinitions().certId);
						refreshItems();
						}
	...
	public void openPlayerBank(Player victim, boolean second) {
		if (victim == null) {
			return;
		}
		player.getInterfaceManager().sendInterface(762);
		player.getInterfaceManager().sendInventoryInterface(763);
		player.getPackets().sendItems(95, !second ? victim.getBankT().getContainerCopy() : victim.getSecondBank().getContainerCopy());
		refreshViewingTab();
		refreshTabs();
		unlockButtons();
	}
	...
	public void depositMoneyPouch() {
		int inPouch = player.getPouch().getAmount();
		if (inPouch == 0)
			return;
		
		Item itemInBank = getItem(995);
		
		if (itemInBank != null) {
			if ((itemInBank.getAmount() == Integer.MAX_VALUE)) {
				player.sendMessage("Your bank can hold no more money.");
				return;
			}
			if (inPouch + itemInBank.getAmount() < 0)
				inPouch = Integer.MAX_VALUE - itemInBank.getAmount();
			if (inPouch <= 0 || itemInBank.getAmount() == Integer.MAX_VALUE ||
					(itemInBank.getAmount() + inPouch > Integer.MAX_VALUE)) {
				player.sendMessage("Your bank can't hold any more money.");
				return;
			}
			addItem(995, inPouch, true);
			player.getPouch().setAmount(0);
			player.getPouch().sendScript(false, inPouch);
	...
	public void depositAllEquipment(boolean banking) {
		...
		if (player.getAuraManager().isActivated()) {
			player.getAuraManager().desactive();
			player.sm("<col=D62727>Your aura was deactivated while depositting your equipment.");
			}
	}
	...
	public void openDepositBox() {
		...
		player.getPackets().sendIComponentText(11, 13,
				"Bank of EradicationX - Deposit Box");
	...
	public void openBank() {
		if (player.getControlerManager().getControler() != null) {
			if (player.getControlerManager().getControler().toString().contains("DuelArena")) {
				player.sm("You can't bank during a duel.");
				return;
			}
		}
		player.getBankT().opened = false;
		player.getSecondBank().opened = false;
		opened = true;
		if (player.getSecurityPin() == 0) {
			...
			player.getPackets().sendRunScript(108,
					new Object[] { "Please create a security pin now! Make sure you remember it." });				
		} else {
		bonestoOffer.stopOfferGod = true;
		if (!player.isBankEquip()) {
		containsItem(1);
		player.getInterfaceManager().sendInterface(762);
		player.getInterfaceManager().sendInventoryInterface(763);
		refreshViewingTab();
		...
		refreshLastX();
			...
			ButtonHandler.openEquipmentBonuses2(player, true);
		}
		}
	}
	...
	public void withdrawItem(int bankSlot, int quantity) {
		...
		if ((item.getId() != 995 && item.getId() != 1464 &&
				item.getId() != 12852 && item.getId() != 19819 &&
						item.getId() != 2996)) {
		if (noted || defs.isStackable()) {
			...
			case 995:
				checked = true;
				if (!player.addMoney(item.getAmount())) {
