...
public class SquealOfFortune implements Serializable {
    ...
    public boolean processItemClick(Item item) {
    	if (item.getId() == 24154 || item.getId() == 24155) { // spin ticket and double spin ticket
    		player.getInventory().deleteItem(item);
    		giveEarnedSpins(item.getId() == 24154 ? 1 : 2);
    ...
    private void obtainReward(boolean discard) {
		...
		if (!discard) {
			if (type == SOF_STATUS_CLAIMPOUCHOK) {
				player.getInventory().addItem(rewards[rewardSlot].getId(), rewards[rewardSlot].getAmount());
    ...
    private Item generateReward(int spinType, int rarityType) {
    	boolean isLamp = Utils.random(3) == 0; // lamp 1/3 others 2/3
    		...
    		if (itemId == 995) {
    			...
    			amount = rarityType > RARITY_COMMON || (!defs.isStackable()) ? 1 : (Utils.random(150));
    		}
	...
	if (reward.getId() == 995) { // coins go to pouch OK
	    long amt = player.getPouch().getAmount() + reward.getAmount();
    ...
    public void giveDailySpins() {
    	if ((Utils.currentTimeMillis() - lastDailySpinsGiveaway) < (1000 * 60 * 60 * 12)) // 12 hours
    		return;
    	...
    	if (dailySpins > 4) // max limit of daily spins bitconfig is 8
    		dailySpins = 4;
	
    	if (dailySpins > previous) {
    		if (player.isDonator())
    			player.getPackets().sendGameMessage("<col=FF0000>You have been awarded " + (dailySpins - previous) + " Squeal of Fortune Spins for being " + (player.isSavior() ? "Super Donator" : player.isExtremeDonator() ? "Extreme Donator" : "Donator") + ".");
    		else
