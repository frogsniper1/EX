...
public final class QueenBlackDragonController extends Controler {
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		if (object.getId() == 70815) {
			player.setSpellPower(player.getSpellPower()+7);
			if (player.getBossTask() != null) {
                if (player.getBossTask().getNPCId() == 15509) {
                    player.getSkills().addXp(Skills.SLAYER, player.getBossTask().getXPAmount());
                    player.getBossTask().decreaseAmount();
                    if (player.getBossTask().getTaskAmount() < 1) {
                    		if (!player.getBossTask().isTripled()) {
                            player.setBossSlayerPoints(player.getBossSlayerPoints() + 20);
                            player.sm("You have finished your slayer task, talk to Grim Reaper for a new task.");
                            player.sm("Grim Reaper rewarded you 20 Boss Slayer Points. You now have "
                                    + player.getBossSlayerPoints()
                                    + " Boss Slayer Points.");
                    		} else {
                                player.setBossSlayerPoints(player.getBossSlayerPoints() + 60);
                                player.sm("You have finished your slayer task, talk to Grim Reaper for a new task.");
                                player.sm("Grim Reaper rewarded you 60 Boss Slayer Points. You now have "
                                        + player.getBossSlayerPoints()
                                        + " Boss Slayer Points.");                    			
                    		}
                    		player.setBossSlayerCount(player.getBossSlayerCount(false) + 1);
                    		player.sm("You now have "+ player.getBossSlayerCount(false) +" boss slayer tasks completed.");
                            player.setBossTask(null);
                    } else {
                    player.getBossTask().setAmountKilled(player.getBossTask().getAmountKilled() + 1);
