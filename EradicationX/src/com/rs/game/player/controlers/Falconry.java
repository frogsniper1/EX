...
public class Falconry extends Controler {
	...
	public static void beginFalconry(Player player) {
		if ((player.getEquipment().getItem(3) != null && player.getEquipment()
				.getItem(3).getId() == -1)
	...
	public boolean processNPCClick1(final NPC npc) {
		...
		if (npc.getDefinitions().name.toLowerCase().contains("kebbit")) {
			...
			if (proccessFalconAttack(npc)) {
				if (player.getSkills().getLevel(Skills.HUNTER) < level) {
					...
					return true;
				} else if (Hunter.isSuccessful(player, level,
						new DynamicFormula() {
							@Override
					...
					player.getTemporaryAttributtes()
							.put("falconReleased", true);
					WorldTasksManager.schedule(new WorldTask() {
						@Override
						public void run() {
							World.sendProjectile(player, npc, 918, 41, 16, 31,
									35, 16, 0);
							WorldTasksManager.schedule(new WorldTask() {
								@Override
								public void run() {
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							...
							WorldTasksManager.schedule(new WorldTask() {
								@Override
								public void run() {
									World.sendProjectile(npc, player, 918, 41,
											16, 31, 35, 16, 0);
									WorldTasksManager.schedule(new WorldTask() {
										@Override
										public void run() {
											player.getEquipment().getItems()
													.set(3, new Item(10024, 1));
											player.getEquipment().refresh(3);
		...
		} else if (npc.getDefinitions().name.toLowerCase().contains(
				"gyr falcon")) {
			NPC kill = (NPC) player.getTemporaryAttributtes()
					.get("ownedFalcon");
			if (kill == null)
				return false;
			if (kill != npc) {
	...
	private boolean proccessFalconAttack(NPC target) {
		...
		if ((!player.clipedProjectile(target, maxDistance == 0))
			...
			if (!player.addWalkStepsInteract(target.getX(), target.getY(), 2,
					size, true))
				return true;
		}
		return true;
