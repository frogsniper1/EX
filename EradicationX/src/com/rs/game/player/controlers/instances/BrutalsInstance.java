...
import com.rs.game.npc.instances.InstanceBrutals;
...
public class BrutalsInstance extends Controler {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(3183, 5715, 0);

	public static void enterTrioInstance(Player player) {
		player.getControlerManager().startControler("BrutalsInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(336, 1187, boundChuncks[0], boundChuncks[1], 8);	
    	...
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (player.getEnd() == true) {
					...
					if (count == 1) {
						try {
						World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(15), getWorldTileY(12), 0), true);
						} catch (Throwable t) {
							Logger.handle(t);
						}
					}
	...
	public void startWave() {
		try {
		new InstanceBrutals(5362, getSpawnTile(0));
		new InstanceBrutals(5362, getSpawnTile(1));
		new InstanceBrutals(5362, getSpawnTile(2));
		new InstanceBrutals(5362, getSpawnTile(3));
		new InstanceBrutals(5362, getSpawnTile(4));
		new InstanceBrutals(5362, getSpawnTile(5));
		new InstanceBrutals(5362, getSpawnTile(6));
		new InstanceBrutals(5362, getSpawnTile(7));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
	...
	public WorldTile getSpawnTile(int num) {
		...
		case 3:
			return getWorldTile(26, 18);
		case 4:
			return getWorldTile(29, 18);
		case 5:
			return getWorldTile(22, 21);
		case 6:
			return getWorldTile(22, 24);
		case 7:
			return getWorldTile(21, 25);
		default:
			return getWorldTile(31, 32);
		}
	}
