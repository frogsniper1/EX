...
import com.rs.game.npc.instances.InstanceExtreme;
...
public class ExtremeInstance extends Controler {
	...
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(2873, 5206, 0);

	public static void enterTrioInstance(Player player) {
		player.getControlerManager().startControler("ExtremeInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(357, 650, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(16), getWorldTileY(5), 0), true);	
				} catch (Throwable t) {
					Logger.handle(t);
				}
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(15, 5));
	...
	public void startWave() {
		try {
		new InstanceExtreme(5561, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
