...
import com.rs.game.npc.instances.InstanceFear;
...
public class FearInstance extends Controler {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(3436, 3732, 0);

	public static void enterTrioInstance(Player player) {
		player.getControlerManager().startControler("FearInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(428, 465, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(24), getWorldTileY(21), 0), true);
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(25, 22));
	...
	public void startWave() {
		try {
		new InstanceFear(15172, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
