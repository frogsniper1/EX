...
import com.rs.game.npc.instances.InstanceGeno;
...
public class GenoInstance extends Controler {
	...
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(3235, 5499, 0);

	public static void enterTrioInstance(Player player) {
		player.getControlerManager().startControler("GenoInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(405, 685, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(16), getWorldTileY(14),  0), true);
					} catch (Throwable t) {
						Logger.handle(t);
					}
					}
	...
	public void startWave() {
		try {
		new InstanceGeno(15006, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
			...
			public void run() {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {	
						...
						player.unlock();
						player.setNextWorldTile(getWorldTile(17, 14));					
						World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(16), getWorldTileY(14),  0), true);						
						startWave();
						startTimer();
