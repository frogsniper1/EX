...
import com.rs.game.npc.instances.InstanceNecrolord;
...
public class NecrolordInstance extends Controler {
	...
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(2646, 10425, 0);

	public static void enterTrioInstance(Player player) {
		player.getControlerManager().startControler("NecrolordInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(329, 1302, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(15), getWorldTileY(11),  0), true);
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(15, 10));
	...
	public void startWave() {
		try {
		new InstanceNecrolord(11751, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
