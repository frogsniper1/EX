...
import com.rs.game.npc.instances.InstanceSTQ;
...
public class STQInstance extends Controler {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(3000, 3128, 0);

	public static void enterTrioInstance(Player player) {
		player.getControlerManager().startControler("STQInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(372, 387, boundChuncks[0], boundChuncks[1], 8);	
	...
	public void startWave() {
		try {
		new InstanceSTQ(3847, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
