...
import com.rs.game.npc.instances.InstanceEradKing;
...
public class EradKingInstance extends Controler {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(3182, 5713, 0);

	public static void enterTrioInstance(Player player) {
		player.getControlerManager().startControler("EradJadInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(420, 400, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(16), getWorldTileY(30), 0), true); 
					} catch (Throwable t) {
						Logger.handle(t);
					}
					}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(15, 31));
	...
	public void startWave() {
		try {
		new InstanceEradKing(1895, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
