...
import com.rs.game.npc.instances.InstanceHairymonkey;
...
public class HairymonkeyInstance extends Controler {
	...
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(3317, 9377, 0);

	public static void enterTrioInstance(Player player) {
		player.getControlerManager().startControler("HairymonkeyInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(412 , 1169, boundChuncks[0], boundChuncks[1], 8);	
		player.isInsideHairymonkey = true;
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.sm("You lose 5 minutes from the timer because of your death.");
					player.setTimer(player.getTimer() - 300);
				} else if (loop == 3) {			
					player.reset();
	...
	public void startWave() {
		try {
		new InstanceHairymonkey(15961, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
	...
	public void exitInstance(boolean isTimeEnd) {
		player.isInsideHairymonkey = false;
