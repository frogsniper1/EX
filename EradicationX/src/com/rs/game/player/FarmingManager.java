...
public class FarmingManager implements Serializable {
    ...
    private static final int ALLOTMENT = 0, TREES = 1, HOPS = 2, FLOWERS = 3,
	    FRUIT_TREES = 4, BUSHES = 5, HERBS = 6, COMPOST = 7, RAKE = 5341,
	    EMPTY_BUCKET = 1925;
	...
	if (!spot.isCleared()) {
		...
		else if (WaterFilling.isFilling(player, item.getId(), true) && player.getInventory().containsItemToolBelt(5325))
		    return;
		else
	...
	if (!player.getToolbelt().contains(isTree ? 952 : 5343)) {
	...
	if (!spot.isDiseased()) {
	    ...
	    if (!(player.getToolbelt().contains(5329) || player.getToolbelt().contains(7409))) {
	...
	player.getActionManager().setAction(new Action() {
		...
		for (HatchetDefinitions def : HatchetDefinitions.values()) {
		    if (player.getToolbelt().contains(def.getItemId()) || player.getEquipment().getWeaponId() == def.getItemId()) {
			hatchet = def;
