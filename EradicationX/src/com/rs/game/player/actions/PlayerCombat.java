...
public class PlayerCombat extends Action {
	...
	private int getRangeCombatDelay(int weaponId, int attackStyle) {
		...
		if (weaponId != -1) {
			...
			if (weaponName.contains("shortbow")
				...
				delay = 5;
			else if (weaponName.contains("deathtouched"))
				delay = 6;
				...
				case 15703:
				case 15704:
				case 28009: //awsome bow have now spec :D
					delay = 9;
					break;
				default:
					delay = 6;
			...
			case 27:// blood barrage
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						delayMagicHit(2, getMagicHit(player, damage));
						World.sendProjectile(player, target, 373, 18, 18, 50, 50, 0, 0);
						if(!nextTarget) {
							if(damage == -1)
								return false;
							nextTarget = true;
	...
	public int getRandomMagicMaxHit(Player player, int baseDamage) {
		...
		if (hit > 0) {
			if (AmuletofCompletionEquipped(player)) {
				if (Utils.getRandom(4) == 1) {
					heal = hit / 5;
					Hit healdisp = new Hit(player, heal, HitLook.HEALED_DAMAGE);
					player.applyHit(healdisp);
				}
			}
			if (target instanceof NPC) {
				NPC n = (NPC) target;
	...
	private int getMagicMaxHit(Player player, int baseDamage) {
		...
		if (player.fullEObsidianEquipped(Equipment.EOBSIDIAN_MHELM))
			EA *= 1.4;
		else if (player.fullObsidianEquipped(26132))
			EA *= 1.3;
		EA *= player.getAuraManager().getMagicAccurayMultiplier();
		double ED = 0, DB;
		if (FullSlayerHelmEquipped(player)) {
		if (target instanceof NPC) {
			if (player.getBossTask() != null) {
				NPC npc = (NPC) target;
				if (npc.getId() == player.getBossTask().getNPCId() 
						|| (npc.getId() == player.getBossTask().getNPCId())) {
					EA *= 1.15;
				}
			}
		}
	...
	private double getMagicAccuracy(Player e, int baseDamage, int extraDamage) {
		...
		if (e.fullEObsidianEquipped(Equipment.EOBSIDIAN_MHELM)) {
			accuracy *= 1.109;
		} else if (e.fullObsidianEquipped(Equipment.OBSIDIAN_MHELM)) {
			accuracy *= 1.106;
		}
	...
	private int rangeAttack(final Player player) {
		...
		case 29980:
		case 29897:
			player.setNextAnimation(new Animation(15241));
			...
			case 28009: //awsome bow have now spec :D
				...
				break;
			case 29910: //decimation
				int ammoId1 = player.getEquipment().getAmmoId();
				player.setNextAnimation(new Animation(getWeaponAttackEmote(
						weaponId, attackStyle)));
				player.setNextGraphics(new Graphics(getArrowThrowGfxId(
						weaponId, ammoId1), 0, 100));
				if (ammoId1 == 29909) {
					int damage = getRandomMaxHit(player, weaponId, attackStyle,
							true, true, 1.75, true);
					if (damage < 300)
						damage = 250;
					int damage2 = getRandomMaxHit(player, weaponId,
							attackStyle, true, true, 1.75, true);
					if (damage2 < 300)
						damage2 = 200;
					World.sendProjectile(player, target, 1099, 41, 16, 31, 35,
							16, 0);
				...
				if (weaponName.contains("throwing axe")
					...
					if (weaponName.contains("deathtouched dart")) {
					    if (target instanceof Player) {
							...
							player.getEquipment().refresh(3);	
								...
								NPC n = (NPC) target;
						    	if (n.getId() != 9622 && n.getId() != 15235 && n.getId() != 7891 && n.getId() != 15976) {
								World.sendProjectile(player, n, 3429, 41, 36, 50, 0, 15, 0);
								n.setNextGraphics(new Graphics(3428));
					            n.setCapDamage(30000);
					            if (n.getId() >= 15977  && n.getId() <= 15979)
					            	n.setCapDamage(8000);
								...
								player.getEquipment().refresh(3);	
						    		...
						    		n.setNextForceTalk(new ForceTalk("Your death touched darts don't affect me! I am Death!"));
						    	}
						    	}
					    }				
				} else if (weaponName.contains("crossbow")) {
					...
					if (ammoId != -1 && Utils.getRandom(10) == 5) {
						...
						case 9242:
							if (target instanceof NPC) {
								NPC n = (NPC) target;
								if (n.getId() == 7891 || n.getId() == 15978 || n.getId() == 15977 || n.getId() == 15979)
									break;
							}
							max_hit = Short.MAX_VALUE;
							damage = (int) (target.getHitpoints() * 0.1);
					...
					if (!player.isDonator() && !player.isExtremeDonator()) {
						player.getPackets().sendGameMessage("You have to be extreme donator to be using Bolas.");
						return combatDelay;
					}
					dropAmmo(player, -3);
					...
					if (weaponId == 11235 || weaponId == 15701
							|| weaponId == 15702 || weaponId == 15703 || weaponId ==29910
							|| weaponId == 15704 || weaponId == 28009) { // dbows
						World.sendProjectile(player, target,
	...
	private int meleeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 26198:
				player.setNextAnimation(new Animation(11971));
				target.setNextGraphics(new Graphics(6));
				target.setNextGraphics(new Graphics(5));
				if (target instanceof Player) {
					Player p2 = (Player) target;
					p2.setRunEnergy(p2.getRunEnergy() > 25 ? p2.getRunEnergy() - 25
							: 0);
			...
			case 25112:
			case 29896:// lance
				player.setNextAnimation(new Animation(15623));
				player.setNextGraphics(new Graphics(44)); //92 // 121
				player.setNextGraphics(new Graphics(902));	
				 if (target instanceof Player) {
				delayNormalHit(
				weaponId,
				attackStyle,
			...
			case 25654: // wand
			case 29899:
				player.setNextAnimation(new Animation(15624));
				player.setNextGraphics(new Graphics(121));
				 if (target instanceof Player) {
				target.setNextAnimation(new Animation(12851));
				target.setNextGraphics(new Graphics(92));
				 } else {
				target.setNextGraphics(new Graphics(92));
				 }
				int lightningDamage2 = getMaxHit(player, weaponId, attackStyle,
						false, true, 1);
				double staffmultiplier2 = 0.2 + Math.random();
				lightningDamage2 *= staffmultiplier2;	
				int lightningDamage3 = getMaxHit(player, weaponId, attackStyle,
						false, true, 1);
				double staffmultiplier3 = 0.2 + Math.random();
				lightningDamage3 *= staffmultiplier3;	
				int lightningDamage4 = getMaxHit(player, weaponId, attackStyle,
						false, true, 1);
				double staffmultiplier4 = 0.2 + Math.random();
				lightningDamage4 *= staffmultiplier4;	
				 if (target instanceof Player) {
				delayNormalHit(weaponId, attackStyle,		
						getMagicHit(player, lightningDamage2));
				delayNormalHit(weaponId, attackStyle,
						getMagicHit(player, lightningDamage3));
				 } else {
				delayNormalHit(weaponId, attackStyle,
						getMagicHit(player, lightningDamage2));	
				delayNormalHit(weaponId, attackStyle,		
						getMagicHit(player, lightningDamage3));
				delayNormalHit(weaponId, attackStyle,
						getMagicHit(player, lightningDamage4));				
				 }
				break;									
			case 13901:
			case 28818:
			case 28813:
				...
				player.setNextGraphics(new Graphics(559));				
				blood_spell = true;
				delayMagicHit(1,
						getMagicHit(player, getRandomMagicMaxHit(player, 250)));
				World.sendProjectile(player, target, 374, 18, 18, 50, 50, 0, 0);				
			...
			case 26787:
				...
				player.setNextGraphics(new Graphics(306));
				target.setNextGraphics(new Graphics(293));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
			...
			case 18786:
			case 19780: // right korasi's sword!
			...
			case 28045: //draconic claws
				player.setNextAnimation(new Animation(10961));
				player.setNextGraphics(new Graphics(1950));
				int[] dchits = new int[] {0, 1};
				int dchit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true);
				if (dchit > 0) {
					dchits = new int[] {dchit, dchit / 2, (dchit / 2) / 2, (dchit / 2) - ((dchit / 2) / 2)};
					target.setNextGraphics(new Graphics(5));
					target.setNextAnimation(new Animation(7299));
					target.setNextForceTalk(new ForceTalk("Arghhhhh!"));					
				} else {
					dchit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true);
					if (dchit > 0) {
						dchits = new int[] {0, dchit, dchit / 2, dchit - (dchit / 2)};
					target.setNextGraphics(new Graphics(5));
					target.setNextAnimation(new Animation(7299));
					target.setNextForceTalk(new ForceTalk("Arghhhhh!"));						
					} else {
						dchit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true);
						if (dchit > 0) {
							dchits = new int[] {0, 0, dchit / 2, (dchit / 2) + 10};
					target.setNextGraphics(new Graphics(5));
					target.setNextAnimation(new Animation(7299));
					target.setNextForceTalk(new ForceTalk("Arghhhhh!"));							
						} else {
							dchit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true);
							if (dchit > 0) {
								dchits = new int[] {0, 0, 0, (int) (dchit * 1.5)};
					target.setNextGraphics(new Graphics(5));
					target.setNextAnimation(new Animation(7299));
					target.setNextForceTalk(new ForceTalk("Arghhhhh!"));								
							} else {
								dchits = new int[] {0, 0, 0, Utils.getRandom(7)};								
							}
						}
					}
				}
				for (int i = 0; i < dchits.length; i++) {
					if (i > 1) {
						delayHit(1, weaponId, attackStyle, getMeleeHit(player, dchits[i]));
					} else {
						delayNormalHit(weaponId, attackStyle, getMeleeHit(player, dchits[i]));
					}
				}
				break;				
			case 10887: // anchor
			...
			case 17275: //frostbite
				...
				soundId = 2537;
				break;
			case 25472: // lightning staff
			case 25473: // lightning staff
			case 25474: // lightning staff
			case 25475: // lightning staff
				player.setNextAnimation(new Animation(12804));
				player.setNextGraphics(new Graphics(166));
				target.setNextAnimation(new Animation(2563));
				target.setNextGraphics(new Graphics(365));
				int lightningDamage = getMaxHit(player, weaponId, attackStyle,
						false, true, 1);
				double staffmultiplier = 0.5 + Math.random();
				max_hit = (int) (lightningDamage * 1.5);
				lightningDamage *= staffmultiplier;
				delayNormalHit(weaponId, attackStyle,
						getMagicHit(player, lightningDamage));
				break;			
			case 29911: // Annihilation
				player.setNextAnimation(new Animation(12033));
				target.setNextGraphics(new Graphics(92));
				delayNormalHit(
						weaponId,
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle,
		...
		if (defenceAffects) {
			...
			if (player.fullEObsidianEquipped(ranging ? Equipment.EOBSIDIAN_RHELM : Equipment.EOBSIDIAN_WHELM))
				att *= 1.2;
			else if (player.fullObsidianEquipped(ranging ? 26130 : 26128))
				att *= 1.1;
			if (ranging)
	...
	private final int getMaxHit(Player player, int weaponId,
		...
		if (!ranging) {
			...
			if (player.fullEObsidianEquipped(Equipment.EOBSIDIAN_WHELM)) 
				effectiveStrength = Math.floor(effectiveStrength*1.2);
			else if (player.fullObsidianEquipped(26128)) 
				effectiveStrength = Math.floor(effectiveStrength*1.1);
			if (SlayerHelmEquipped(player)) {
				if (target instanceof NPC) {
					if (player.getBossTask() != null) {
						NPC npc = (NPC) target;
						if (npc.getId() == player.getBossTask().getNPCId()) {
			...
			if (player.fullWObsidianEquipped(Equipment.WOBSIDIAN_WHELM)) 
				effectiveStrength = Math.floor(effectiveStrength*1.2);
			else if (player.fullObsidianEquipped(26128)) 
				effectiveStrength = Math.floor(effectiveStrength*1.1);
			if (SlayerHelmEquipped(player)) {
			...
			if (fullObsidianEquipped(player) && target instanceof Player) 
				effectiveStrength = Math.floor(effectiveStrength/1.4);
			if (player.fullEObsidianEquipped(Equipment.EOBSIDIAN_WHELM) && target instanceof Player) 
				effectiveStrength = Math.floor(effectiveStrength/2.7);
			if (weaponId == 28045 && target instanceof Player)
				effectiveStrength = Math.floor(effectiveStrength/1.3);
			/*if (player.getEquipment().getHatId() == 
			 * 
			 */
			if (weaponId == 26198 && target instanceof Player)
				effectiveStrength = Math.floor(effectiveStrength/1.3);
			...
			if (player.fullEObsidianEquipped(Equipment.EOBSIDIAN_RHELM))
				effectiveStrenght += Math.floor((player.getSkills().getLevelForXp(
						Skills.RANGE) / 5) + 1.7);
			else if (player.fullObsidianEquipped(26130))
				effectiveStrenght += Math.floor((player.getSkills().getLevelForXp(
						Skills.RANGE) / 5) + 1.6);
	...
	public static final boolean fullDharokEquipped(Player player) {
		...
		public static final boolean fullObsidianEquipped(Player player) {
		int chestId = player.getEquipment().getChestId();
		int legsId = player.getEquipment().getLegsId();
		if (chestId == -1 || legsId == -1)
			return false;
		return ItemDefinitions.getItemDefinitions(chestId).getName()
				.contains("Obsidian")
				&& ItemDefinitions.getItemDefinitions(legsId).getName()
				.contains("Obsidian");
	}
		
		public static final boolean AmuletofCompletionEquipped(Player player) {
		int ammyId = player.getEquipment().getAmuletId();
		if (ammyId == -1)
			return false;
		if (ammyId == 6194)
			return true;
		...
		public static final boolean FullSlayerHelmEquipped(Player player) {
			if (player.getEquipment().getHatId() == 15492
					|| player.getEquipment().getHatId() == 22528
					|| player.getEquipment().getHatId() == 22534
					|| player.getEquipment().getHatId() == 22540
					|| player.getEquipment().getHatId() == 22546)
				return true;
			return false;
		}			
					...
					int damage = hit.getDamage() > target.getHitpoints() ? target
										...
										if (weaponId != -1) {
											...
											if (name.contains("(p++)")) {
												if (Utils.getRandom(8) == 0)
													target.getPoison().makePoisoned(48, false);
											} else if (name.contains("(p+)")) {
												if (Utils.getRandom(8) == 0)
													target.getPoison().makePoisoned(38, false);
											} else if (name.contains("(p)")) {
												if (Utils.getRandom(8) == 0)
													target.getPoison().makePoisoned(28, false);
											}
										}
									} else {
										if (weaponId != -1) {
											...
											if (name.contains("(p++)")) {
												if (Utils.getRandom(8) == 0)
													target.getPoison().makePoisoned(68, false);
											} else if (name.contains("(p+)")) {
												if (Utils.getRandom(8) == 0)
													target.getPoison().makePoisoned(58, false);
											} else if (name.contains("(p)")) {
												if (Utils.getRandom(8) == 0)
													target.getPoison().makePoisoned(48, false);
											}
							...
							if (max_poison_hit > 0 && Utils.getRandom(10) == 0) {
								if (!target.getPoison().isPoisoned())
									target.getPoison().makePoisoned(max_poison_hit, false);
							}
		...
		if (weaponId != -1) {
			...
			if (weaponName != null && !weaponName.equals("null")) {
				...
				if (weaponName.contains("bow"))
					return 426;
				if (weaponName.contains("decimation"))
					return 426;
				...
				if (weaponName.contains("obliteration") || weaponName.contains("staff") || weaponName.contains("wand"))
					return 419;
				...
				if (weaponName.contains("scimitar")
						|| weaponName.contains("korasi's sword") || weaponName.contains("drygore longsword")) {
					switch (attackStyle) {
					case 2:
						return 15072;
					default:
				...
				if (weaponName.contains("annihilation")) {
					switch (attackStyle) {
					case 2:
						return 400;
					default:
				...
				if (weaponName.contains("tzhaar-ket-om") || weaponName.contains("sword of edicts")) {
					switch (attackStyle) {
					default:
						return 13691;
					}
				...
				if (weaponName.contains("2h sword")
						...
						|| weaponName.equals("shadow sword")
						|| weaponName.equals("shadow sword")
				...
				if (weaponName.contains("katana") || weaponName.contains("blazing")) {
					switch (attackStyle) {
					case 2:
						return 1882;
					default:
				...
				if (weaponName.contains("greataxe") || weaponName.contains("christmas")) {
					switch (attackStyle) {
					case 2:
						return 12003;
					default:
	...
	private int getMeleeCombatDelay(Player player, int weaponId) {
		if (weaponId != -1) {
			...
			if (weaponName.contains("godsword")
					...
					|| weaponName.contains("sword of edicts")
					|| weaponName.equals("dominion sword"))
				return 5;
			// Interval 4.2
	...
	public static final int isRanging(Player player) {
		...
		case 22348: //dom c bow
		case 24575: // quickfire c bow ;) 
			switch (ammoId) {
			case -1:
				return 3;
			case 877: // bronze bolts
