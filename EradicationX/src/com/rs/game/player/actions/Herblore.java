...
public class Herblore extends Action {
	...
	public enum Ingredients {
		...
		ERADICATORPOT(15332, new short[] { Herblore.ENCHANTING_POTION, Herblore.SWAMP_TAR },
				new short[] { 29974, 10142 }, new byte[] { 0, 19 }, new double[] {
						0, 30 }),
						
		ROCKTAILSOUP(15272, new short[] { 2003, Herblore.SWAMP_TAR },
				new short[] { 26313, 10142 }, new byte[] { 0, 19 }, new double[] {
				0, 30 }),							
		...
		TORSTOL(269, new short[] { Herblore.VIAL, 15309, 15313, 15317, 15321,
				15325 },
				new short[] { 111, 15332, 15332, 15332, 15332, 15332 },
		...
		MORCHELLA_MUSHROOM(21622, new short[] { Herblore.VIAL, 21624 },
				new short[] { 21632 }, new byte[] { 94 },
				new double[] { 190.0 }),
	...
	public static final short ENCHANTING_POTION = 29976;
	...
	public boolean process(Player player) {
		...
		if (otherItem.getId() == 12539 && node.getId() == RANGE_POT3) {
			if (!player.getInventory().containsOneItem(RANGE_POT3)
					|| !player.getInventory().containsOneItem(12539))
				return false;
			return true;
		}
		if (otherItem.getId() == MUD_RUNE && node.getId() == PESTLE_AND_MORTAR) {
			if (!player.getInventory().containsOneItem(MUD_RUNE)
					|| !player.getInventory()
							.containsOneItem(PESTLE_AND_MORTAR))
				return false; 
			return true; 
		}
	...
	public int processWithDelay(Player player) {
		...
		boolean isCrushingMudRune = false;
		if (node.getId() == PESTLE_AND_MORTAR
		...
		if (otherItem.getId() == VIAL || otherItem.getId() == COCONUT_MILK
			...
			player.getPackets().sendGameMessage(
					"You add the"
							+ otherItem.getDefinitions().getName()
									.toLowerCase().replace("clean", "")
							+ " into the vial of "
							+ (node.getId() == VIAL ? "water." : "milk."));
		...
		} else if (otherItem.getId() == MUD_RUNE
				&& node.getId() == PESTLE_AND_MORTAR) {
			isCrushingMudRune = true;
			player.getPackets().sendGameMessage(
					"You crush the "
							+ otherItem.getDefinitions().getName()
									.toLowerCase()
			...
			if (player.getInventory().removeItems(new Item(15309),
					new Item(15313), new Item(15317), new Item(15321),
					new Item(15325))) {
			}
		} else {
			player.getPackets()
		...
		if (isOverload == false && isCrushingMudRune == false) {
			player.getInventory().removeItems(
					new Item(node.getId(), 1),
					rawIngredient == null ? new Item(otherItem.getId(), 1)
							: null);
		} else if (isCrushingMudRune == false) {
			player.getInventory().removeItems(new Item(otherItem.getId(), 1));
			isOverload = false;
		} else if (isCrushingMudRune == true) {
			player.getInventory().removeItems(new Item(otherItem.getId(), 1));
			isCrushingMudRune = false;
		}
