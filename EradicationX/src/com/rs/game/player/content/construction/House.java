...
public class House {
	...
	public void constructHouse(int[] boundChuncks, boolean buildMode) {
		...
		Region region = World.getRegion(RegionBuilder.getRegionHash(
		...
		for (RoomReference reference : rooms) {
			...
			int chunkRotation = region.getRotation(reference.plane, boundX,
					boundY);
			for (int x = boundX; x < boundX + 8; x++) {
				for (int y = boundY; y < boundY + 8; y++) {
					WorldObject[] objects = region.getObjects(reference.plane,
							x, y);
					if (objects != null) {
	...
	public void previewRoom(Player player, int[] boundChuncks,
			RoomReference reference, boolean remove) {
		int boundX = reference.x * 8;
		int boundY = reference.y * 8;
		Region region = World.getRegion(RegionBuilder.getRegionHash(
		...
		DynamicRegion house = (DynamicRegion) World.getRegion(RegionBuilder
				.getRegionHash(boundChuncks[0] / 8, boundChuncks[1] / 8));
