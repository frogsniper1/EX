...
public class Hunter {
	...
	public static void openJar(Player player, int id) {
		switch (id) {
		case 6055:
				player.addMoney(20000);
				player.getSkills().addXp(HUNTER, 60);
			break;
		case 6056:
			if (player.getSkills().getLevel(HUNTER) >= 22) {
			player.addMoney(50000);
			player.getSkills().addXp(HUNTER, 65);
			} else {
				player.sm("You need a hunter level of 22 to catch this imp.");
				return;
			}
			break;
		case 6057:
			if (player.getSkills().getLevel(HUNTER) >= 28) {
			player.addMoney(70000);
			player.getSkills().addXp(HUNTER, 70);
			} else {
				player.sm("You need a hunter level of 28 to catch this imp.");
				return;
			}
			break;
		case 6058:
			if (player.getSkills().getLevel(HUNTER) >= 36) {
			player.addMoney(100000);
			player.getSkills().addXp(HUNTER, 75);
			} else {
				player.sm("You need a hunter level of 36 to catch this imp.");
				return;
			}
			break;
		case 6059:
			if (player.getSkills().getLevel(HUNTER) >= 42) {
			player.addMoney(120000);
			player.getSkills().addXp(HUNTER, 80);
			} else {
				player.sm("You need a hunter level of 42 to catch this imp.");
				return;
			}
			break;
		case 6060:
			if (player.getSkills().getLevel(HUNTER) >= 50) {
			player.addMoney(140000);
			player.getSkills().addXp(HUNTER, 85);
			} else {
				player.sm("You need a hunter level of 50 to catch this imp.");
				return;
			}
			break;
		case 6061:
			if (player.getSkills().getLevel(HUNTER) >= 58) {
			player.addMoney(160000);
			player.getSkills().addXp(HUNTER, 90);
			} else {
				player.sm("You need a hunter level of 58 to catch this imp.");
				return;
			}
			break;
		case 6062:
			if (player.getSkills().getLevel(HUNTER) >= 65) {
			player.addMoney(180000);
			player.getSkills().addXp(HUNTER, 95);
			} else {
				player.sm("You need a hunter level of 65 to catch this imp.");
				return;
			}
			break;
		case 6063:
		case 6053:
			if (player.getSkills().getLevel(HUNTER) >= 74) {
			player.addMoney(200000);
			player.getSkills().addXp(HUNTER, 100);
			} else {
				player.sm("You need a hunter level of 74 to catch this imp.");
				return;
			}
			break;
		case 6064:
		case 6054:
			if (player.getSkills().getLevel(HUNTER) >= 83) {
			player.addMoney(220000);
			player.getSkills().addXp(HUNTER, 105);
			} else {
				player.sm("You need a hunter level of 83 to catch this imp.");
				return;
			}
			break;
		}
		player.setNextAnimation(CAPTURE_ANIMATION);
		player.sm("You capture the "+ NPCDefinitions.getNPCDefinitions(id).getName() + ".");
		player.lock(3);
	}
