...
public class FlowerGame {
	...
	public static void plant(final Player player) {
		if (player.trustedflower = false) {
			player.sm("You are an untrusted host you canno't play.");
			return;
		}
		player.setNextAnimation(new Animation(827));

		World.spawnTempGroundObject(new WorldObject(randomFlower(), 10, 0,
				player.getX(), player.getY(), player.getPlane()), 299, 35000);
	...
	public static void cheat(final Player player) {
		...
		World.spawnTempGroundObject(new WorldObject(2983, 10, 0, player.getX(),
				player.getY(), player.getPlane()), 299, 35000);
