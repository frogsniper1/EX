...
public class Dicing {
	...
	public static void friendsRoll(final Player player, final int itemId,
		...
		if (!player.isDicer()) {
			player.sm("Only a dicer can dice. Become a dicer today by purchasing the rank.");
			return;
		}
		player.lock(2);
		player.getInventory().deleteItem(itemId, 1);
		...
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				int random = getRandom(lowest, highest);
				Logger.printDiceLog(player, random);
				player.getInventory().addItem(itemId, 1);
				player.setRolls(random);
				chat.sendDiceMessage(
						player,
						"[<img=11>Dicer]: <col=db3535>"
