...
public class RegularPlayer {
	public static boolean hasPrivelages(Player player) {
		if (player.getRights() >= 1
				|| player.getRights() == 0 && player.isExtremeDonator()
				|| player.getRights() == 0 && player.isLentDonator()		
				|| player.getRights() == 0 && player.isLentExtreme()	
				|| player.getRights() == 0 && player.isLentSavior()
				|| player.getRights() == 0 && player.isLentEradicator()		
				|| player.getRights() == 0 && player.isEradicator()
				|| player.getRights() == 0 && player.isDonator()
				|| player.getRights() == 0 && player.isForumMod()
				|| player.getRights() == 0 && player.isDicer()
	...
	public static boolean processCommand(Player player, String[] cmd, boolean console, boolean clientCommand) {
		if (clientCommand) {
			...
			if (player.isIronMan() || player.getRights() > 6) {
				if (cmd[0].equals("ironzone") || cmd[0].equals("iz") 
						|| cmd[0].equals("ironmanzone") 
			...
			if (cmd[0].equals("checktbox")) { 
				int[] trimmedItems = {6617,6619,6623,6625,6629,4125,2583,2585,2587,2589,2591,
			...
			if (cmd[0].equals("hidedice")) {
				...
				player.getPackets().sendGameMessage("You have " + (player.isDiceAnnounce() ? "locked" : "unlocked") + " your dice announcements.");
				return true;
			}
			if (cmd[0].equals("changepin")) {
				...
				player.getPackets().sendRunScript(108,
						new Object[] { "Please enter your current pin number:" });	
				return true;
			}
			...
			if (cmd[0].equals("ctitlename")) {
				if (player.getClanManager().getClan().getClanLeaderUsername().equalsIgnoreCase(player.getUsername())) {
					...
					player.getPackets().sendRunScript(109,
							new Object[] { "Please enter the title name you want" });
				} else {
					player.sm("You must be a clan leader to access these commands.");
				}
				return true;
			...
			if (player.getClanName() != null) {
					...
					if (player.getClanManager().getClan().getClanLeaderUsername().equalsIgnoreCase(player.getUsername()))
						player.sm("You're the clan leader. You may edit the clan title via ;;ctitlecolor, ;;ctitlename, ;;ctitleshade, ;;ctitlenamecolor.");
			...
			if (player.inInstance()) {
				...
				player.sm("You can't use this command when you're not in an instance. You noob. What'd the command even do if you weren't in an instance?");
			}
				return true;
			}	
			...
			player.getPackets().sendIComponentText(275, 15, "<col=ED0C17>If found scamming, you will be confiscated of the item,");
			...
			player.getPackets().sendIComponentText(275, 19, "<col=ED0C17>If a glitch is found, kindly report it on its section.");
			player.getPackets().sendIComponentText(275, 20, "<col=ED0C17> Otherwise, you will be banned for abusing the glitch.");
			player.getPackets().sendIComponentText(275, 22, "<col=ED0C17><u>3. No Staff Impersonation:");
			player.getPackets().sendIComponentText(275, 23, "<col=ED0C17>Change of Name Display to resemble a Staff Member is not tolerated.");
			player.getPackets().sendIComponentText(275, 24, "<col=ED0C17>You will be requested to remove the name,");
			...
			player.getPackets().sendIComponentText(275, 28, "<col=ED0C17>Do not spam other player's game-chat or advertise another server,");
			player.getPackets().sendIComponentText(275, 29, "<col=ED0C17>you will be muted for spamming, and IPbanned for Advertising.");
			player.getPackets().sendIComponentText(275, 31, "<col=ED0C17><u>5. No AFKing:");
			player.getPackets().sendIComponentText(275, 32, "<col=ED0C17>Anyone seen AFK skilling will result in a sendhome at first, then jail.");
			player.getPackets().sendIComponentText(275, 34, "<col=ED0C17><u>6. No Disrespecting Staff:");
			player.getPackets().sendIComponentText(275, 35, "<col=ED0C17>Disrespecting our dedicated staff will result in the following:");
			...
			player.getPackets().sendIComponentText(275, 41, "<col=ED0C17>Multilogging is not allowed, you will be asked to");
			...
			player.getPackets().sendIComponentText(275, 45, "<col=ED0C17> No Auto-clicking levels (Jail), or use of any bots");
			player.getPackets().sendIComponentText(275, 46, "<col=ED0C17>Auto-Typing is allowed if used every 5+ seconds");
			player.getPackets().sendIComponentText(275, 48, "<col=ED0C17><u>9. No Safespotting:");
			player.getPackets().sendIComponentText(275, 49, "<col=ED0C17>Safe spotting bosses (exception of Corp) is not acceptable.");
			player.getPackets().sendIComponentText(275, 50, "<col=ED0C17>first offense will be a warning, subsequent offenses will result in a jail.");
			player.getPackets().sendIComponentText(275, 52, "<col=ED0C17><u>10. No Real World Trading (RWT):");
			player.getPackets().sendIComponentText(275, 53, "<col=ED0C17>RWT is not tolerated. Instant IP ban will be applied on your account.");
			player.getPackets().sendIComponentText(275, 55, "<col=ED0C17><u>11. Buying/Selling Runescape Items/accounts:");
			player.getPackets().sendIComponentText(275, 56, "<col=ED0C17> Not tolerated. Only form of Runescape related item ");
			player.getPackets().sendIComponentText(275, 57, "<col=ED0C17>exchanging will be for donation purposes only.");
			player.getPackets().sendIComponentText(275, 59, "<col=ED0C17><u>12. No Flaming:");
			player.getPackets().sendIComponentText(275, 60, "<col=ED0C17>You will be granted two warnings, then you will be muted.");
			player.getPackets().sendIComponentText(275, 62, "<col=ED0C17><u>13. No Flame Baiting:");
			player.getPackets().sendIComponentText(275, 63, "<col=ED0C17>If you are caught attempting to provoke a player into flaming,");
			...
			player.getPackets().sendIComponentText(275, 67, "<col=ED0C17> Any form of threats to DDoS the server/players is not tolerated.");
			...
			player.getPackets().sendIComponentText(275, 71, "<col=ED0C17> Voting for over 10 tickets in a day is not tolerated. A moderator");
			player.getPackets().sendIComponentText(275, 72, "<col=ED0C17> has permission to revoke your tickets from your account.");
			player.getPackets().sendIComponentText(275, 74, "<col=ED0C17><u>16. Jail Evading:");
			player.getPackets().sendIComponentText(275, 75, "<col=ED0C17> Evading jail by logging onto an alternate account WILL RESULT");
			...
			player.getPackets().sendIComponentText(275, 80, "<col=ED0C17>actively fighting back, will result in a jail.");
			player.getPackets().sendIComponentText(275, 82, "<col=0C7682> If you have any questions, do ::ticket");
			player.getPackets().sendIComponentText(275, 83, "<col=0C7682> If you have one of these punishments and wish to ");
			player.getPackets().sendIComponentText(275, 84, "<col=0C7682> appeal, go on our forums and make a thread.");
			player.getPackets().sendIComponentText(275, 85, "<col=0C7682>");
			player.getPackets().sendIComponentText(275, 86, "<col=0C7682><img=7><img=13><img=16><img=18<img=0> EradicationX Staff <img=7><img=13><img=16><img=18<img=0>");
			...
			player.getPackets().sendIComponentText(275, 15, "<col=ED0C17>Any other method of dicing will cause the gambler");
			player.getPackets().sendIComponentText(275, 16, "<col=ED0C17>to get banned. Only trust <img=11>Dice Ranks");
			player.getPackets().sendIComponentText(275, 17, "<col=ED0C17>If a rank doesn't pay out, report it to a staff");
			player.getPackets().sendIComponentText(275, 18, "<col=ED0C17>Immediately. No proof is required if said in chat.");		
			return true;
			}			
            ...
            if (cmd[0].equals("yell") || cmd[0].equals("y")) {
				if (IPMute.isMuted(player.getSession().getIP())) {
					player.getPackets().sendGameMessage(
							"Your account has been temporarily IP Muted.");
					return true;
				}
			...
			if (cmd[0].equals("checkmbox")) {
				int[] trimmedItems = {1037,14595,14603,1050,23679,23680,23681,11700,
			...
			if (cmd[0].equals("kdr")) {
				...
				player.setNextForceTalk(new ForceTalk(
						"<col=ff0000>I'VE KILLED " + player.getKillCount()
								+ " PLAYERS AND DIED "
								+ player.getDeathCount() + " TIMES. KDR: " + dr));
				return true;
			}
			...
			if (cmd[0].equalsIgnoreCase("claimweb") || 
					cmd[0].equalsIgnoreCase("donated") || 
					cmd[0].equalsIgnoreCase("claimreward") ||
					cmd[0].equalsIgnoreCase("paid")) {
				if (player.isIronMan()) {
					player.sm("Ironman accounts cannot donate. If you choose to ignore this warning and donate regardless, we are not held responsible!");
					return true;
				}
				try {
			...
			if (cmd[0].equalsIgnoreCase("cw") ||
					cmd[0].equalsIgnoreCase("clanwars") ||
					cmd[0].equalsIgnoreCase("pk") ||
			...
			if (cmd[0].equalsIgnoreCase("dicezone") ||
					cmd[0].equalsIgnoreCase("dice")) {
				if (!player.canSpawn()) {
					player.getPackets().sendGameMessage("You can't teleport while you're in this area.");
					return true;
				}			
			...
			if (cmd[0].equalsIgnoreCase("upgrade1") || cmd[0].equalsIgnoreCase("upgrade2") || cmd[0].equalsIgnoreCase("upgrade3")
					|| cmd[0].equalsIgnoreCase("upgrade4") || cmd[0].equalsIgnoreCase("upgrade5") || cmd[0].equalsIgnoreCase("upgrade6") || cmd[0].equalsIgnoreCase("upgrade7")) {
				player.sm("This command was removed. Please right click your whip to upgrade it.");
				return true;
			}	
			if (cmd[0].equalsIgnoreCase("staff1") || cmd[0].equalsIgnoreCase("staff2") || cmd[0].equalsIgnoreCase("staff3")) {
