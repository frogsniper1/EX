...
public class ClansManager {
	...
	public static void kickClanmate(Player player) {
		...
		if (member.getRank() == Clan.LEADER) {
			player.getPackets().sendGameMessage("You can't kick the leader!");
			return;
		}
	...
	public static void saveClanmateDetails(Player player) {
		...
		synchronized (manager) {
			...
			SerializableFilesManager.saveClan(manager.clan);
		}
	}
	...
	public void sendMessage(Player player, ChatMessage message) {
		synchronized (this) {
			...
			for (Player p2 : channelPlayers)
				p2.getPackets().receiveClanChatMessage(membersOnline.contains(p2), "<img="+player.getMessageIcon()+">"+displayName, -1, message);
		}
	}
	...
	public static int sizeClans() {
		return cachedClans.size();
	}
	...
	public static void openClanDetails(Player player, Player p2, ClansManager manager) {
		...
		if (manager.clan.getMottifTop() != 0)
			player.getPackets().sendSpriteOnIComponent(1107, 96, getMottifSprite(manager.clan.getMottifTop()));
		if (manager.clan.getMottifBottom() != 0)
			player.getPackets().sendSpriteOnIComponent(1107, 106, getMottifSprite(manager.clan.getMottifBottom()));
