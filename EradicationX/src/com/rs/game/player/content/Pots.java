...
public final class Pots {
	...
	public static enum Pot {
		...
		//SUPER_PRAYER_POTION(new int[] { 2434, 139, 141, 143 }, Effects.SUPER_PRAYER_POTION),
		...
		TELEBLOCK_BREAK(new int[] { 29978}, Effects.TELEBLOCK_BREAK),
		ERADICATOR(new int[] { 29974}, Effects.ERADICATOR),
	...
	private enum Effects {
		...
		TELEBLOCK_BREAK() {
			@Override
			public void extra(Player player) {
				player.getTemporaryAttributtes().remove("TeleBlocked");
				player.getPackets().sendGameMessage(
						"Your Teleport Block was successfully removed.");
			}
		},
		ENERGY_POTION() {
			@Override
		...
		ERADICATOR() {
			...
			public boolean canDrink(Player player) {
				...
				if (player.getHitpoints() <= 100
						|| player.getEradicatorDelay() > 80) {
					player.getPackets()
							.sendGameMessage(
									"You need more than 100 life points to survive the power of the Eradicator Potion.");
					return false;
				}
				return true;
			}		
			...
			public void extra(final Player player) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						player.getAppearence().transformIntoNPC(13955);
	...
	public static void resetEradicatorEffect(Player player) {
		if (!player.isDead()) {
			int actualLevel = player.getSkills().getLevel(Skills.ATTACK);
			int realLevel = player.getSkills().getLevelForXp(Skills.ATTACK);
			if (actualLevel > realLevel)
		...
		player.setEradicatorDelay(0);
		player.getAppearence().transformIntoNPC(-1);
		player.getPackets()
				.sendGameMessage(
						"<col=480000>The effects of the Eradicator Potion have worn off and you feel normal again.");
	}	
	
	public static void applyEradicatorEffect(Player player) {
		if (player.getControlerManager().getControler() instanceof Wilderness
				|| player.getControlerManager().getControler() instanceof CrucibleControler
				|| FfaZone.isOverloadChanged(player)) {
			int actualLevel = player.getSkills().getLevel(Skills.ATTACK);
			...
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().set(Skills.RANGE,
					(int) (level + 4 + (Math.floor(realLevel / 5.9))));
		}
	}
