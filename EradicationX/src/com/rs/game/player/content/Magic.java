...
public class Magic {
	...
	public static final boolean checkCombatSpell(Player player, int spellId,
		...
		case 193:
			...
			case 36: // Miasmic rush.
				...
				if (weaponId != 13867 && weaponId != 13869 && weaponId != 13941 && weaponId != 13943 && !player.isExtremeDonator() || !player.isLentExtreme() || !player.isLentSavior() 
						|| !player.isLentEradicator()|| !player.isEradicator() || !player.isSavior()) {
			...
			case 38: // Miasmic burst.
				...
				if (weaponId != 13867 && weaponId != 13869 && weaponId != 13941
						&& weaponId != 13943 && !player.isExtremeDonator()) {
					player.getPackets().sendGameMessage(
							"You need a Zuriel's staff to cast this spell.");
					player.getPackets()
							.sendGameMessage(
			...
			case 39: // Miasmic barrage.
				...
				weaponId = player.getEquipment().getWeaponId();
				if (weaponId != 13867 && weaponId != 13869 && weaponId != 13941 && weaponId != 25472 && weaponId != 25473 && weaponId != 25474 && weaponId != 25475
						&& weaponId != 13943 && !player.isExtremeDonator() && !player.isEradicator() && !player.isSavior() && player.isLentExtreme() && player.isLentSavior() && player.isLentEradicator()) {
					player.getPackets().sendGameMessage(
							"You need a Zuriel's staff to cast this spell.");
					player.getPackets()
							.sendGameMessage(
	 ...
	 public static void handleMagicOnItem(Player player, int magicId, Item item) {
	        ...
	        switch (magicId) {
	            ...
	            case 29: // lvl 1 enchant
	                break;
	                
	            case 41: // lvl 2 enchant
	                break;
	                
	            case 53: // lvl 3 enchant
	                break;
	                
	            case 61: // lvl 4 enchant
	                break;
	                
	            case 76: // lvl 5 enchant
	                break;

	            case 88: // lvl 6 enchant
	                break;
	        }
	    }
	    ...
	    public static void processAlchemy(Player player, Item item, boolean low) {
	    	...
	    	if (player.getActionManager().getActionDelay() > 0)
	    		return;
	        int baseValue = PriceManager.getSellPrice(new Item(item.getId()));
	        int value = baseValue / (low == true ? 2 : 5);
	        if (!player.hasCoinSpace(value)) {
	        	player.sm("You do not have enough coin space to alch this item.");
	        	return;
        	}
	        if (value <= 1) {
	        	player.sm("This item has no value!");
	        	return;
	        }
	        ...
	        if (item.getId() == 995 || !ItemConstants.isTradeable(item)) {
	            player.getPackets().sendGameMessage("You can't alch this!");
	            return;
	        }
	        if (!checkRunes(player, true, FIRE_RUNE, low ? 3 : 5, NATURE_RUNE, 1)) {
	        ...
	        if (low)
		        ...
		        player.getSkills().addXp(Skills.MAGIC, 12);
	        Logger.printAlchLog(player, item, value, new Item(995));
	    }	
	...
	private static void useHomeTele(Player player) {
		player.stopAll();
		sendNormalTeleportSpell(player, 0, 0, Settings.RESPAWN_PLAYER_LOCATION);
	}
	...
	public static final void sendNormalTeleportSpell(Player player, int level,
		...
		sendTeleportSpell(player, 17118, 8941, 3227, 678, level, xp, tile, 5,
					true, MAGIC_TELEPORT, runes);
		} else if (player.isSavior() || player.isLentSavior()){
		sendTeleportSpell(player, 4977, 8941, 2846, 2845, level, xp, tile, 7/3,
				true, MAGIC_TELEPORT, runes);
		} else {
		sendTeleportSpell(player, 1128, 8941, 2602, 2603, level, xp, tile, 5,
				true, MAGIC_TELEPORT, runes);
		}
	...
	public static final void sendObjectTeleportSpell(Player player,
		...
		sendTeleportSpell(player, 17106, 8941, 3223, 1577, 0, 0, tile, 8,
