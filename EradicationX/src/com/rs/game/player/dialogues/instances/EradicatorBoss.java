...
public class EradicatorBoss extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaEradicator(bool);
	}
	private String getString = "EradicatorInstance";
	public void startControler() {
		player.getControlerManager().startControler("EradicatorInstance");	
	}
	public EradicatorBoss() {
	}
	
	@Override
	public void start() {
		player.instanceDialogue = "Eradicator";
		player.setOutside(new WorldTile(2944, 4389, 0));
		stage = 1;
	...
	public void run(int interfaceId, int componentId) {
		boolean getPerma = player.getPermaEradicator();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2944, 4389, 0));
