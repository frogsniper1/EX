...
public class Gradum extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaGradum(bool);
	}
	private String getString = "GradumInstance";
	public void startControler() {
		player.getControlerManager().startControler("GradumInstance");	
	}
	public Gradum() {
	}
	...
	@Override
	public void start() {
		player.instanceDialogue = "Gradum";
		player.setOutside(new WorldTile(2901, 4397, 0));
		stage = 1;
	...
	public void run(int interfaceId, int componentId) {
		boolean getPerma = player.getPermaGradum();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2901, 4397, 0));
