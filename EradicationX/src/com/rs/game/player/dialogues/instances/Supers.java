...
public class Supers extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaSupers(bool);
	}
	private String getString = "SupersInstance";
	public void startControler() {
		player.getControlerManager().startControler("SupersInstance");	
	}
	public Supers() {
	}
	...
	@Override
	public void start() {
		player.instanceDialogue = "Supers";
		player.setOutside(new WorldTile(3362, 3340, 0));
		stage = 1;
	...
	public void run(int interfaceId, int componentId) {
		boolean getPerma = player.isPermaSupers();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2344, 5608, 0));
