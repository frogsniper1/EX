...
public class Rajj extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaRajj(bool);
	}
	private String getString = "RajjInstance";
	public void startControler() {
		player.getControlerManager().startControler("RajjInstance");	
	}
	public Rajj() {
	}
	...
	@Override
	public void start() {
		player.instanceDialogue = "Rajj";
		player.setOutside(new WorldTile(4991, 4195, 3));
		stage = 1;
	...
	public void run(int interfaceId, int componentId) {
		boolean getPerma = player.getPermaRajj();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(4991, 4195, 3));
