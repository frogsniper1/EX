...
public class STQ extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaSTQ(bool);
	}
	private String getString = "STQInstance";
	public void startControler() {
		player.getControlerManager().startControler("STQInstance");	
	}
	public STQ() {
	}
	...
	@Override
	public void start() {
		player.instanceDialogue = "STQ";
		player.setOutside(new WorldTile(3000, 3128, 0));
		stage = 1;
	...
	public void run(int interfaceId, int componentId) {
		boolean getPerma = player.getPermaSTQ();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3000, 3128, 0));
