...
public class Wyrm extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaWyrm(bool);
	}
	private String getString = "InstanceWyrm";
	public void startControler() {
		player.getControlerManager().startControler("InstanceWyrm");	
	}
	public Wyrm() {
	}
	...
	@Override
	public void start() {
		player.instanceDialogue = "Wyrm";
		player.setOutside(new WorldTile(3968, 4823, 1));
		stage = 1;
	...
	public void run(int interfaceId, int componentId) {
		boolean getPerma = player.getPermaWyrm();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3159, 3719, 0));
