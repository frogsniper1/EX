...
public class Extreme extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaExtreme(bool);
	}
	private String getString = "ExtremeInstance";
	public void startControler() {
		player.getControlerManager().startControler("ExtremeInstance");	
	}
	public Extreme() {
	}
	...
	@Override
	public void start() {
		player.instanceDialogue = "Extreme";
		player.setOutside(new WorldTile(2870, 5204, 0));
		stage = 1;
	...
	public void run(int interfaceId, int componentId) {
		boolean getPerma = player.isPermaExtreme();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2870, 5204, 0));
