...
public class Brutals extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaBrutals(bool);
	}
	private String getString = "BrutalsInstance";
	public void startControler() {
		player.getControlerManager().startControler("BrutalsInstance");	
	}
	public Brutals() {
	}
	...
	@Override
	public void start() {
		player.instanceDialogue = "Brutals";
		player.setOutside(new WorldTile(3183, 5715, 0));
		stage = 1;
	...
	public void run(int interfaceId, int componentId) {
		boolean getPerma = player.isPermaBrutals();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2329, 5608, 0));
