...
public class EradKing extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaObbyKing(bool);
	}
	private String getString = "EradKingInstance";
	public void startControler() {
		player.getControlerManager().startControler("EradKingInstance");	
	}
	public EradKing() {
	}
	...
	@Override
	public void start() {
		player.instanceDialogue = "EradKing";
		player.setOutside(new WorldTile(3182, 5713, 0));
		stage = 1;
	...
	public void run(int interfaceId, int componentId) {
		boolean getPerma = player.isPermaObbyKing();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2359, 5559, 0));
