...
public class Hairymonkey extends Dialogue {
	

	private String getString = "HairymonkeyInstance";
	...
	public void start() {
		...
		sendOptionsDialogue("What would you like to do?",
				"Start a private instance", "Nevermind");
	}
	...
	public void run(int interfaceId, int componentId) {
		...
		if (stage == 1) {	
			if (componentId == OPTION_1) {
					sendDialogue("A new session will start once you",
							"continue for 30 minutes. Do you wish to start?",
							"Note: You only lose the instance if the exit portal is clicked.",
							"If you die, you lose 5 minutes and spawn back into the zone.");
				stage = 2;					
			} else {
				end();
			}
		...
		stage = 9;					
			...
			player.sm("You don't have a key.");
			end();	
		} else if (stage == 10) {
			Item item = player.getInventory().containsAtleastItem(new int[]{29947, 27749, 27750, 27751, 27752, 27753});
			if (item != null) {
				player.getInventory().deleteItem(item);
				player.sm("Enjoy your instance, the Hairymonkey boss will stop spawning in 30 minutes.");
				startControler();
			} else {
				player.sm("You don't have a key.");
			}
