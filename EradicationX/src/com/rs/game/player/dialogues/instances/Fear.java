...
public class Fear extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaFear(bool);
	}
	private String getString = "FearInstance";
	public void startControler() {
		player.getControlerManager().startControler("FearInstance");	
	}
	public Fear() {
	}
	...
	@Override
	public void start() {
		player.setOutside(new WorldTile(3436, 3732, 0));
		stage = 1;
	...
	public void run(int interfaceId, int componentId) {
		boolean getPerma = player.getPermaFear();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3436, 3732, 0));
