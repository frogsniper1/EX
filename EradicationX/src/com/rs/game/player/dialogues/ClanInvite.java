...
public class ClanInvite extends Dialogue {
	...
	public void start() {
		...
		if (manager.getClan().getMottifTop() != 0)
			player.getPackets().sendSpriteOnIComponent(1095, 44, ClansManager.getMottifSprite(manager.getClan().getMottifTop()));
		if (manager.getClan().getMottifBottom() != 0)
			player.getPackets().sendSpriteOnIComponent(1095, 54, ClansManager.getMottifSprite(manager.getClan().getMottifBottom()));
	}
