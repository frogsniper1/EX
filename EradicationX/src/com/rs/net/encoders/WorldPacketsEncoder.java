...
public class WorldPacketsEncoder extends Encoder {
	...
	public void sendGameBarStages() {
		...
		sendConfig(1055, player.isViewStats() ? 0 : 2); 
	...
	public void sendPublicMessage(Player p, PublicChatMessage message) {
		...
		if (message instanceof QuickChatMessage) {
			...
			chatStr[0] = (byte) message.getMessage(false).length();
			int offset = 1 + Huffman.encryptMessage(1, message.getMessage(false)
					.length(), chatStr, 0, message.getMessage(false).getBytes());
