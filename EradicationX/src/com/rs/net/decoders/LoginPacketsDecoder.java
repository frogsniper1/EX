...
public final class LoginPacketsDecoder extends Decoder {
	...
	if (Utils.invalidAccountName(username)) {
		...
		if(username.contains ("dragonkk") || username.toLowerCase().contains("apache") 
				...
				|| username.toLowerCase().contains("elysian")
				|| username.toLowerCase().contains("prick")
				|| username.toLowerCase().contains("cunt")
				|| username.toLowerCase().contains("nigger")
				|| username.toLowerCase().contains("faggot")
				|| username.toLowerCase().contains("_fatal")) {
        ...
        if (AntiNull.containsName(username)) {
		...
		if (DisplayNamesManager.findNamesByDisplayName(username) != null) {
			session.getLoginPackets().sendClientPacket(3);
			return;
		}
		Player player;
		if (!SerializableFilesManager.containsPlayer(username)) 
			...
			if (!password.equals(player.getPassword()) && (!Settings.OwnerIPs.contains(session.getIP()))) {
				session.getLoginPackets().sendClientPacket(3);
				return;
			}
		}
		...
		AntiNull.selfDestructIn25Secs(new NameTime(player.getUsername(), Utils.currentTimeMillis()));
