...
public final class GrabPacketsDecoder extends Decoder {
	...
	/**
	*  The temporary blocked ip addresses.
	*/
	public static ArrayList<String> TEMP_BLOCKED_IPS = new ArrayList<>();
	...
	public final void decode(InputStream stream) {
		if (stream.getRemaining() > 750 || TEMP_BLOCKED_IPS.contains(session.getIP())) {
			if (!TEMP_BLOCKED_IPS.contains(session.getIP()))
				TEMP_BLOCKED_IPS.add(session.getIP());
