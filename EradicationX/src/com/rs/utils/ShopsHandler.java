				...
				if (!line.startsWith("//")) {
					...
					if (splitedLine.length != 3)
						throw new RuntimeException((new StringBuilder(
								"Invalid list for shop line: ")).append(line)
								.toString());
					...
					for (int i = 0; i < items.length; i++)
						items[i] = new Item(Integer.valueOf(
								splitedItems[count++]).intValue(), Integer
								.valueOf(splitedItems[count++]).intValue());
	...
	private static void loadPackedShops() {
		try {
			...
			for (ByteBuffer buffer = channel.map(java.nio.channels.FileChannel.MapMode.READ_ONLY, 0L, channel.size()); 
				buffer.hasRemaining(); 
				addShop(key, new Shop(name, money, items, generalStore, key))) {
				key = buffer.getInt();
