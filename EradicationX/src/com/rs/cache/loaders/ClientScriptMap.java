...
public final class ClientScriptMap {
	...
	public static void dumpCsMap(int scriptId) throws IOException {
		Store cache = new Store(Settings.CACHE_PATH);
		byte[] data = cache.getIndexes()[17].getFile(scriptId >>> 0xba9ed5a8, scriptId & 0xff);
		if (data != null) {
			FileOutputStream fos = new FileOutputStream(System.getProperty("user.home") + "/desktop/csmap " + scriptId +".dat");
			fos.write(data);
			fos.flush();
			fos.close();
		}
	...
	public static void putCsMapFromFile(int scriptId, String filePath) throws IOException {
		Store cache = new Store(Settings.CACHE_PATH);
		cache.getIndexes()[17].putFile(scriptId >>> 0xba9ed5a8, scriptId & 0xff, Utils.getBytesFromFile(new File(System.getProperty("user.home") + "/desktop/csmap" + scriptId + ".dat")));
	}	
